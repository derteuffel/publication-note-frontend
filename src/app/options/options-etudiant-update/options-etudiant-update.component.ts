...
@Component({
  ...
  templateUrl: './options-etudiant-update.component.html',
  styleUrls: ['./options-etudiant-update.component.css']
})
export class OptionsEtudiantUpdateComponent implements OnInit {
  ...
  onSubmit(){
    this.etudiantService.updateEtutiant(this.etudiant, this.etudiant.id).subscribe(
      data => {
        ...
        this.route.navigate(['/options/etudiants/detail/'+this.etudiant.id]);
      },
      error => {
        console.log(error);
      }
